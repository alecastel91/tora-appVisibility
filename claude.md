...
### Beta test infrastructure (Aug 27) — frontend half
- **BetaSheet** (`components/common/BetaSheet.js`, opened by the BetaTools FAB, beta-only): two tabs. YOUR LIST — grouped task rows from GET /beta/tasks, polls every 15s while open (auto-ticks appear without reload), "Show me how" hints, "This was confusing" → prefilled feedback, skip-with-reason; auto-detected rows aren't manually toggleable. TELL US — type/severity chips, up to 5 downscaled screenshots, silent context (route, active tab, commit via /api/health cached on `window.__toraCommit`, device, `window.__toraLastApiError` set by api.js handleResponse, Sentry.lastEventId).
- **Strings** live OUTSIDE the main i18n catalog in `src/beta/tasks-strings.js` (BETA_TASKS_EN/JA ×52 + BETA_UI en/ja; other locales fall back to EN; JA needs Alessandro's native pass). Generated from tora-backend/scripts/data/beta-world.json.
- **Pitch path (BI6, PRODUCT feature ×8 locales)**: artist/agent viewing a venue/promoter profile gets "Send a pitch" (chat if connected, else connect flow) + one-line rule explainer (`pitch.*` keys) — offers stay one-directional by design.
