...
- **Install CTA at signup**: `InstallSheet` bottom sheet appears once (900ms after first authed load, browser-only, localStorage-dismissed) — Chromium gets a one-tap native install via the captured `beforeinstallprompt` (`src/services/install.js`, imported for side effect from index.js so the listener exists before Chrome fires the event); iOS gets 2-step Add-to-Home-Screen instructions; icon matches the surface (BETA-badged on beta). `PushNudge` no longer renders its ios-needs-install variant — install education lives in the sheet, the nudge only shows where push is actually enableable ('ready', i.e. inside the installed app or desktop). i18n `install.*` ×8.
