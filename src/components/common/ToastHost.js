...
const ToastHost = () => {
  ...
  return (
    <OverlayPortal>
      {/* Bottom glass pill above the tab bar; z-index above every overlay
          (likes often happen from the full-screen ViewProfile, which must
          not cover the toast). Tap to dismiss. */}
      <div
        className="pointer-events-none fixed inset-x-0 z-[9999] flex justify-center px-6"
        style={{ bottom: 'calc(env(safe-area-inset-bottom) + 84px)' }}
      >
        <div
          className="pointer-events-auto max-w-sm rounded-full border border-white/15 bg-[#101015]/95 px-4 py-2.5 text-center text-[13px] leading-snug text-white/90 shadow-lg backdrop-blur-md"
